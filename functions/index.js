...
app.post("/payment/payumoney", function (req, res) {
	...
	const hashString = key + "|" + pd.txnid + "|" + pd.amount + "|" + pd.productinfo + "|" + pd.firstname + "|" + pd.email + "|" + "||||||||||" + salt; // Your salt value
...
app.post("/payment/payumoney/response", function (req, res) {
	...
	const keyString = key + "|" + txnid + "|" + amount + "|" + productinfo + "|" + firstname + "|" + email + "||||||||||";
	...
	if (calchash == resphash) {
		...
		res.send({ msg: msg, txnid: txnid, amount: amount, firstname: firstname, mihpayid: mihpayid, status: status, mode: mode, productinfo: productinfo, email: email });
	} else {
		res.send("err");
	}
});
exports.app = functions.region("asia-south1").https.onRequest(app);
exports.sendMail = functions.region("asia-south1").https.onRequest((req, res) => {
	corsHandler(req, res, () => {
		const det = req.body;
