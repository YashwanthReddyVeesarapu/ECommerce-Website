...
export const handleFetchProducts = ({
  ...
  return new Promise((resolve, reject) => {
    ...
    if (searh !== undefined && searh !== null) pageSize = 12;
    ...
    if (cat === "apparel") {
      ...
      if (filterType === "mens" || filterType === "womens")
        ref = ref.where("productCategory", "==", filterType);
    }
    if (cat === "living") {
      ref = ref.where("mainCategory", "==", "living");
      if (filterStore === "rediva") ref = ref.where("brand", "==", "REDIVA");
      if (filterType === "frames" || filterType === "canvas")
        ref = ref.where("productCategory", "==", filterType);
    }
    if (searh !== null && searh !== undefined)
      ref = ref.where("keywords", "array-contains", searh);
        ...
        const data = [
          ...
          ...snapshot.docs.map((doc) => {
            return {
              ...doc.data(),
              documentID: doc.id,
              discountedPrice: Math.trunc(
                doc.data().discount
                  ? (doc.data().productPrice * (100 - doc.data().discount)) /
                      100
                  : null
              ),
            };
