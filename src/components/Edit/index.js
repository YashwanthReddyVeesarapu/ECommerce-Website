...
const Edit = () => {
  ...
  adaptiveThumbnails &&
    Object.keys(adaptiveThumbnails.map).map((val, i) => {
      if (
        i === Object.values(adaptiveThumbnails.map).indexOf(productThumbnail[0])
      )
        console.log(val);
    });
  ...
  return (
    <div>
      <h4>
        ...
        <input
          value={emainCategory}
          onChange={(e) => setEmaincategory(e.target.value)}
        />{" "}
        <br />
        Category:{" "}
        <input
        ...
        <input
          value={ecolors}
          onChange={(e) => setEcolors(e.target.value.toLowerCase().split(","))}
        />{" "}
        <br />
        Discount:{" "}
        <input
        ...
        <textarea
          ...
          rows="5"
          cols="40"
          onChange={(e) => setEthumbnails(e.target.value.split(","))}
        />{" "}
        <br />
        Description :{" "}
        <textarea
        ...
        />{" "}
        <button onClick={generateKeywotds}> Gen </button> <br />
        Existing Id:
        ...
        <input
          value={edocID}
          onChange={(e) =>
            setEdocID(e.target.value.toLowerCase().trim().replaceAll(" ", ""))
          }
        />
        <button
          onClick={() =>
            documentID !== edocID
              ? changeDocID(
                  ename && ename.toLowerCase().trim().replaceAll(" ", "")
                )
        ...
        {ethumbnails.map((src, i) => (
          <div key={i}>
            ...
            <input
              placeholder={
                adaptiveThumbnails &&
                Object.values(adaptiveThumbnails.map).indexOf(src) !== -1
                  ? Object.keys(adaptiveThumbnails.map).map((val, i) => {
                      if (
                        i === Object.values(adaptiveThumbnails.map).indexOf(src)
                      )
                        return val;
                    })
                  : "none"
              }
              onKeyDown={(e) => e.which === 13 && set(e.target.value, src)}
            ></input>
          </div>
        ))}
