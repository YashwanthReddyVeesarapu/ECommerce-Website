...
import { addProduct, handleSize, handleColour } from './../../redux/Cart/cart.actions';
import Button from './../forms/Button';
import ImageSlider from './../Slider/imageSlider';
import './styles.scss';
import { Helmet } from 'react-helmet';
import { MenuItem, Select, FormControl, CircularProgress, InputLabel } from '@material-ui/core';
...
const ProductCard = ({ }) => {
  ...
  useEffect(() => {
    Array.isArray(productColor) && productColor.map((x, i) => {
      if (i === 0)
        setColour(x)
    })
  }, [productColor]);
  ...
  if (productThumbnail !== undefined || adaptiveThumbnails !== undefined || productColor !== undefined) {
    return (
      <div className="productCard">
        <Helmet>
          ...
          <meta property="product:price:amount" content={discountedPrice === 0 ? productPrice : discountedPrice} />
        ...
        <div className="productDetails">
          <div className="mainDetails" >
            <ul>
              <li >
                <p style={{ fontSize: '2.5rem', marginBottom: '5px' }} aria-label="title" >
                  {productName}
                </p>
              </li>
              <li>
            ...
            <div className="productColours">
              {adaptiveThumbnails.colors.map((a, i) => {
                if (a === 'white')
                  return (
                    <div className={colour === a ? "colourChildActive" : "colourChild"} key={i} style={{ backgroundColor: 'white' }} onClick={() => setColour('white')} />

                  )
                if (a === 'black')
                  return (
                    <div className={colour === a ? "colourChildActive" : "colourChild"} key={i} style={{ backgroundColor: 'black' }} onClick={() => setColour('black')} />
                  )
            ...
            <div className='sizeSelect' >
              ...
              <FormControl style={{ minWidth: 80 }} variant="outlined" >
                ...
                <Select style={{ fontSize: 10 }} required value={size} onChange={(e) => setSize(e.target.value)} >
                  ...
                  {productSize.map((name, value) => ({ name: name, value: value })).map((item) => (
                    <MenuItem style={{ fontSize: 10 }} key={item.value} value={item.name}>
                      {item.name}
                    </MenuItem>
              ...
              <SimpleDialog open={open} onClose={handleClose} text={`Please, <br />Select ${size === '' && (colour === '' || colour === 'z') ? "Size & Colour" : (size === '') ? "Size" : "Colour"}<br />To Procced with cart...`} />
            </div>
    ...
    return (
      <CircularProgress style={{ alignSelf: 'center', color: 'inherit', marginLeft: '48%', marginTop: '35vh', marginBottom: '35vh' }} />
