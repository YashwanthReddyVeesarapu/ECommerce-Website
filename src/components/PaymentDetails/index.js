...
const PaymentDetails = () => {
  ...
  const handleOpen = (x) => {
    if (x === 1) setErrorOpen(true);
    if (x === 0) setStatusOpen(true);
  };
      ...
      responseHandler: function (BOLT) {
        apiInstance
          ...
          .then((res) => {
            ...
            } else handleOpen(1); //ALERT {1 = Error, 0 = Status}
          });
        // .then(function (json) {
        //   console.log(json);
        //  });
    ...
    return (
      <div className="paymentDetails">
        <SimpleDialog
          open={errorOpen}
          onClose={handleClose}
          text={"Sorry... <br />Payment FAILED, Retry payment again."}
        />
        <SimpleDialog
          open={statusOpen}
          onClose={handleClose}
          text={`STATUS <br />Payment ${
            status !== "" ? status : "Not Yet initiated"
          } <br /> ${
            paymentID !== "" ? `Transaction ID : ${paymentID}` : "Continue .."
          }`}
        />
